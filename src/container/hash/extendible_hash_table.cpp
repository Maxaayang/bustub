...
bool HASH_TABLE_TYPE::Insert(Transaction *transaction, const KeyType &key, const ValueType &value) {
  ...
  // std::cout << "ret: " << ret << " " << "bucket_page->IsFull(): " << bucket_page->IsFull() << std::endl;
...
void HASH_TABLE_TYPE::Merge(Transaction *transaction, const KeyType &key, const ValueType &value) {
  ...
  if (bucket_page->IsEmpty() && bucket_local_depth > 0) {
    ...
    uint32_t image_depth = directory_page->GetLocalDepth(image_idx);
    page_id_t image_id;
    if (image_depth == bucket_local_depth) {
      merge_occur = true;
      image_id = directory_page->GetBucketPageId(image_idx);
    }

    if (merge_occur) {
      for (uint32_t i = 0; i < dir_size; i++) {
        if ((i & local_mask) == (bucket_page_idx & local_mask)) {
...
bool HASH_TABLE_TYPE::ExtraMerge(Transaction *transaction, const KeyType &key, const ValueType &value) {
  ...
  if (bucket_local_depth > 0) {
    ...
    if (image_local_depth == bucket_local_depth && image_page->IsEmpty()) {
      extra_merge_occur = true;
      page_id_t tmp_bucket_page_id;
      for (uint32_t i = 0; i < dir_size; i++) {
        tmp_bucket_page_id = dir_page->GetBucketPageId(i);
        if (tmp_bucket_page_id == image_id) {
          dir_page->SetBucketPageId(i, bucket_id);
          dir_page->DecrLocalDepth(i);
        } else if (tmp_bucket_page_id == bucket_id) {
          dir_page->DecrLocalDepth(i);
        }
      }
      buffer_pool_manager_->UnpinPage(image_id, false);
