...
class Matrix {
 protected:
   ...
   *
   */
  Matrix(int rows, int cols) : rows_(rows), cols_(cols), linear_(nullptr) { linear_ = new T[rows * cols]; }
...
class RowMatrix : public Matrix<T> {
 public:
  ...
  void FillFrom(const std::vector<T> &source) override {
    ...
    if (vector_size != this->rows_ * this->cols_) {
...
class RowMatrixOperations {
 public:
  ...
  static std::unique_ptr<RowMatrix<T>> Multiply(const RowMatrix<T> *matrixA, const RowMatrix<T> *matrixB) {
    ...
    auto result = std::unique_ptr<RowMatrix<T>>(new RowMatrix<T>(row_a, col_b));

    for (int i = 0; i < row_a; i++) {
      for (int j = 0; j < col_b; j++) {
        T mul = 0;
        for (int k = 0; k < k_size; k++) {
          mul += matrixA->GetElement(i, k) * matrixB->GetElement(k, j);
        }
