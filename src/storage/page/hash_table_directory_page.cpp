...
uint32_t HashTableDirectoryPage::GetGlobalDepthMask() {
  global_depth_mask_ = static_cast<uint32_t>(0x1 << global_depth_) - 1;
  return global_depth_mask_;
}
...
uint32_t HashTableDirectoryPage::GetLocalDepthMask(uint32_t bucket_idx) {
  ...
  local_depth_mask = static_cast<uint32_t>(0x1 << local_depth) - 1;
...
uint32_t HashTableDirectoryPage::Size() {
  auto size = static_cast<uint32_t>(1 << global_depth_);
  return size;
  // return sizeof(bucket_page_ids_[1]) / sizeof(bucket_page_ids_);
}
...
uint32_t HashTableDirectoryPage::GetSplitImageIndex(uint32_t bucket_idx) {
  // uint32_t image_idx = DIRECTORY_ARRAY_SIZE;
  // auto local_mask = GetLocalDepthMask(bucket_idx);
  // auto new_mask = local_mask >> 1;
  // auto dir_size = Size();
  // for (uint32_t i = 0; i < dir_size; i++) {
  //   if (i != bucket_idx && GetLocalDepth(i) == GetLocalDepth(bucket_idx) && (i & new_mask) == (bucket_idx &
  //   new_mask)) {
  //     image_idx = i;
  //     break;
  //   }
...
uint32_t HashTableDirectoryPage::GetLocalHighBit(uint32_t bucket_idx) { return local_depths_[bucket_idx]; }
...
void HashTableDirectoryPage::VerifyIntegrity() {
  ...
  //  verify for each bucket_page_id, pointer，统计每个page id被多少个桶所使用
