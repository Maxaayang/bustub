...
bool BufferPoolManagerInstance::UnpinPgImp(page_id_t page_id, bool is_dirty) {
  ...
  if (frame_id != -1 && pages_[frame_id].GetPinCount() != 0) {
    if (is_dirty) {
    ...
    if (--pages_[frame_id].pin_count_ == 0) {
      ...
      // 这里的 FlushPg 可有可无，因为在 FindFreshPage 还会进行 FlushPg
      FlushPg(page_id);
    }
  }
  return true;
...
frame_id_t BufferPoolManagerInstance::FindPage(page_id_t page_id) {
  std::unordered_map<page_id_t, frame_id_t>::iterator search_iter = page_table_.find(page_id);
